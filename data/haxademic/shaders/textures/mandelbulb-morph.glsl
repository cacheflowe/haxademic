...
float map(vec3 p)
{
    ...
    for(int n=0; n<=MAX_ITER; ++n)
    {
        ...
        float theta = acos(v.z/r) + sin(time);
        float phi = atan(v.y, v.x);
        d = pow(r,Power-1.2)*Power*d+1.1 + sin(time);
...
void main( void )
{
    vec2 pos = (vertTexCoord.xy*2.0 - resolution.xy) / resolution.y;
    vec3 camPos = vec3(cos(time*0.3+time), sin(time*0.3-time), 2.-sin(time*.2)/2.);
