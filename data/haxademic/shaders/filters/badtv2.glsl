...
void main() {
	...
	float red 	=   texture(texture, 	vec2(uv.x + xOffset -0.01*rgbOffsetOpt,y)).r+staticVal;
	float green = 	texture(texture, 	vec2(uv.x + xOffset,	  y)).g+staticVal;
	float blue 	=	texture(texture, 	vec2(uv.x + xOffset +0.01*rgbOffsetOpt,y)).b+staticVal;
