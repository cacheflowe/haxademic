...
void main() {
	...
	// 	colorDisplaced /= 3.;
		...
		vec2 opFlowDisplace = texture2D(map, p).xy - 0.5; // 1/255 is a weird flow correction. the optical flow result never seems to get back to perfect 0.5, but rather 1/255 step away
		opFlowDisplace *= amp;
