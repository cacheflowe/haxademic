...
extends PAppletHax {
	...
	protected void firstFrame() {
		...
		invert = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/filters/invert.glsl" ); 
		...
		edge = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/filters/edge.glsl" ); 
		
		dotScreen = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/filters/dotscreen.glsl" ); 
		dotScreen.set("tSize", 256f, 256f);
		dotScreen.set("center", 0.5f, 0.5f);
		dotScreen.set("angle", 1.57f);
		dotScreen.set("scale", 1f);

		pixelate = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/filters/pixelate.glsl" ); 
		pixelate.set("divider", p.width/20f, p.height/20f);

		radialBlur = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/filters/radial-blur-iq.glsl" ); 
		radialBlur.set("time", _timeEaseInc );
		radialBlur.set("resolution", 1f, (float)(p.width/p.height));
		
		warping = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/filters/warping-iq.glsl" ); 
		warping.set("time", _timeEaseInc );

		deformHoles = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/filters/deform-holes-iq.glsl" ); 
		deformHoles.set("time", _timeEaseInc );
		deformHoles.set("resolution", 1f, (float)(p.width/p.height));
		deformHoles.set("mouse", 0.5f, 0.5f);

		deformRelief = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/filters/fisheye.glsl" ); 
		...
		_image = p.loadImage( FileUtil.haxademicDataPath() + "images/green-screen-2.png" );

		
		glowwave = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/textures/glowwave.glsl" ); 
		swirl = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/textures/swirl.glsl" ); 
		coffeeswirl = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/textures/inversion-iq.glsl" ); 
		clouds = loadShader( FileUtil.haxademicDataPath()+"haxademic/shaders/textures/clouds-iq.glsl" ); 
