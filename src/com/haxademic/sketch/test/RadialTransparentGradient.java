...
extends PAppletHax {
	...
	protected float numSegments = 20f;
	protected float numShapes = 6f;
	protected float frames = 400f;
	...
	public void drawApp() {
		...
		DrawUtil.feedback(p.g, p.color(255), 0.1f, 2f);
...
//		p.rotateZ(-progressRadians);
	...
	//		p.sphere(100);
		...
		for (int i = 0; i < numShapes; i++) {
			...
			float radiusOscillations = 5f + 2f * P.sin(i/3f);
...
//				p.fill(innerColor);
				...
				p.stroke(outerColor);
//				p.fill(outerColor);
				float curRadius = baseRadius + (baseRadius * 0.4f * P.sin(progressRadians + i+r*radiusOscillations));
				float nextRadius = baseRadius + (baseRadius * 0.4f * P.sin(progressRadians + i+r2*radiusOscillations));
...
//		CubicLensDistortionFilter.instance(p).applyTo(p);
		VignetteAltFilter.instance(p).setDarkness(-1);
		VignetteAltFilter.instance(p).setSpread(0.25f);
