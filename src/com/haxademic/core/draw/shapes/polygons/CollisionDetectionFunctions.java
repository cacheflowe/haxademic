...
public class CollisionDetectionFunctions {
  /*
  ...
  public static boolean circleCircle(float c1x, float c1y, float c1r, float c2x, float c2y, float c2r) {
    ...
    float distX = c1x - c2x;
    float distY = c1y - c2y;
    float distance = P.sqrt((distX * distX) + (distY * distY));
  ...
  public static boolean lineCircle(float x1, float y1, float x2, float y2, float cx, float cy, float r) {
    ...
    float distX = x1 - x2;
    float distY = y1 - y2;
    float len = P.sqrt((distX * distX) + (distY * distY));

    // get dot product of the line and circle
    float dot = (((cx - x1) * (x2 - x1)) + ((cy - y1) * (y2 - y1))) / P.pow(len, 2);
  ...
  // POINT/POINT
  public static boolean pointPoint(float x1, float y1, float x2, float y2) {
  ...
  public static boolean polyRect(PVector[] vertices, float rx, float ry, float rw, float rh) {
    ...
    for (int current = 0; current < vertices.length; current++) {
      ...
      boolean inside = polyPoint(vertices, rx, ry);
      if (inside)
        return true;
    }
  ...
  // TRIANGLE/POINT
  public static boolean triPoint(float x1, float y1, float x2, float y2, float x3, float y3, float px, float py) {
    
    // get the area of the triangle
    float areaOrig = P.abs( (x2-x1)*(y3-y1) - (x3-x1)*(y2-y1) );
    
    // get the area of 3 triangles made between the point
    // and the corners of the triangle
    float area1 =    P.abs( (x1-px)*(y2-py) - (x2-px)*(y1-py) );
    float area2 =    P.abs( (x2-px)*(y3-py) - (x3-px)*(y2-py) );
    float area3 =    P.abs( (x3-px)*(y1-py) - (x1-px)*(y3-py) );
