...
public class PShapeOBJVertColors extends PShape {

	/**
	 * Initializes a new OBJ Object with the given filename.
	 */
	public PShapeOBJVertColors(PApplet parent, String filename) {
		this(parent, parent.createReader(filename), getBasePath(parent, filename));
	}

	public PShapeOBJVertColors(PApplet parent, BufferedReader reader) {
		this(parent, reader, "");
	}

	public PShapeOBJVertColors(PApplet parent, BufferedReader reader, String basePath) {
		...
		parseOBJ(parent, basePath, reader,
				faces, materials, coords, colors, normals, texcoords);

		// The OBJ geometry is stored with each face in a separate child shape.
		parent = null;
		family = GROUP;
		addChildren(faces, materials, coords, colors, normals, texcoords);
	}


	protected PShapeOBJVertColors(OBJFace face, OBJMaterial mtl,
		...
		for (int j = 0; j < face.vertIdx.size(); j++){
			...
			// --------------------------------------------------------
			// added by @cacheflowe - use colors collected per vertex if they exist
			if(colors.size() > 0) {
				PVector color = colors.get(vertIdx);
				vertices[j][PGraphics.R] = color.x;
				vertices[j][PGraphics.G] = color.y;
				vertices[j][PGraphics.B] = color.z;
			}
			// --------------------------------------------------------
	...
	protected void addChildren(ArrayList<OBJFace> faces,
		...
		for (int i = 0; i < faces.size(); i++) {
			...
			PShape child = new PShapeOBJVertColors(face, mtl, coords, colors, normals, texcoords);
			addChild(child);
		}
	}
			...
			while ((line = reader.readLine()) != null) {
				...
				if (parts.length > 0) {
					if (parts[0].equals("v")) {
						...
						coords.add(tempv);
						// ---------------------------
						// added by @cacheflowe: read extra params as colors
						if(parts.length == 7 || parts.length == 8) {
							PVector tempc = new PVector(
									Float.valueOf(parts[4]).floatValue(),
									Float.valueOf(parts[5]).floatValue(),
									Float.valueOf(parts[6]).floatValue());
							colors.add(tempc);
						}
						// ---------------------------
						readv = true;
