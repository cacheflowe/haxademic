...
//	protected int COLOR_4 = ColorUtil.colorFromHex("#0c7498");
	...
	protected float textureCycleToDeformLoops = 1;
	protected float animationSeconds = 7.f;
	...
	protected float blobScale = 0.21f;	// 0.36f
	protected float blobDeformAmp = 0.3f; // 0.7f
	protected float spreadMult = 9.6999f; // 2.64277f; 	// 	2.64277f  //	9.165f	//	0.781f	//	5.57249f	//  0.21055f
...
//	protected float wobbleSize = 8f;
	...
	// lighting props
	public float directionLight = 190;
	public float emissiveMaterial = 15f; // 5f
	public float ambientLight = 20f;
	public float specularMaterial = 150f;
	public float specularLight = 250f;
	public float shininessVal = 125f; // 50f
	public float lightsFalloffVal = 0.08f;
	...
	protected void buildSpheres() {
		...
		// wireframe test
		PShape icosWire = Icosahedron.createIcosahedronGrouped(p, icosaDetail, null, p.color(210,255), p.color(50, 80), 0.005f);
...
//		shapeIcos_solid.shape().setTexture(tickerFXBuffer);
		...
		// lights
//		buffer.ambient(127);
		buffer.lightSpecular(130, 130, 130); 
...
//		buffer.lightSpecular(specularLight, specularLight, specularLight);
		...
		// draw solid sphere
		if(DEBUG_MODE == true) { 
			spreadMult = P.map(p.mouseX, 0, p.width, 0.1f, 20f);
			P.println("spreadMult",spreadMult);
		}

		// spin it
		buffer.rotateY(progressRadians);

//		shapeIcos_solid.setVertexColorWithAudio(p.color(255,0,0));
//		shapeIcos_solid.updateWithTrig(false, progress * 1f, 0.12f, 3.f);
		curShape.updateWithTrig(false, progress * textureCycleToDeformLoops, 0.27f, spreadMult);
...
//		shapeIcos_solid.updateWithTrigAndNoiseCombo(progress * P.TWO_PI, 2.17f, spreadMult, 0.75f, 1f, 200f, noiseOctaves, noiseFalloff);
