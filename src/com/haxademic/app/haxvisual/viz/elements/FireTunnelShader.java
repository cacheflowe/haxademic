...
implements IVizElement  {
	...
	public FireTunnelShader( PApplet p, ToxiclibsSupport toxi, AudioInputWrapper audioData ) {
	...
	protected void loadShaders() {
		_patternShader = p.loadShader( FileUtil.getHaxademicDataPath() + "haxademic/shaders/textures/firey-spiral.glsl" ); 
...
//		_kaleido.set("sides", MathUtil.randRange(1, 2));
	...
	public void updateTiming() {
		if( _timingFrame % 4 == 0 ) {
			...
			_timeEaser.setTarget( _timeEaser.value() + 1.3f );
		} else {
			_brightEaser.setCurrent(0.8f);
			_timeEaser.setTarget( _timeEaser.value() + 0.3f );
		}
