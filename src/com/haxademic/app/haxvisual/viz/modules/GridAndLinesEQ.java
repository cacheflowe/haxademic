...
implements IVizModule
{
	...
	{
		// clear screen!
		if ( p.key == 'm' || p.key == 'M' || p.getMidi().midiPadIsOn( MidiWrapper.PAD_04 ) == 1 || p.getMidi().midiPadIsOn( MidiWrapper.NOTE_04 ) == 1 || p.getOsc().oscMsgIsOn( OscWrapper.MSG_MODE ) == 1 ) {
			pickNewColors();
			pickMode();
			newCamera();
		}
		if ( p.key == 'c' || p.key == 'C' || p.getMidi().midiPadIsOn( MidiWrapper.PAD_01 ) == 1 || p.getMidi().midiPadIsOn( MidiWrapper.NOTE_01 ) == 1 || p.getOsc().oscMsgIsOn( OscWrapper.MSG_COLOR ) == 1 ) {
			pickNewColors();
		}
		if ( p.key == 'v' || p.key == 'V' || p.getMidi().midiPadIsOn( MidiWrapper.PAD_02 ) == 1 || p.getMidi().midiPadIsOn( MidiWrapper.NOTE_02 ) == 1 || p.getOsc().oscMsgIsOn( OscWrapper.MSG_CAMERA ) == 1 ) {
			newCamera();
		}
		if ( p.key == ' ' || p.getMidi().midiPadIsOn( MidiWrapper.PAD_07 ) == 1 || p.getMidi().midiPadIsOn( MidiWrapper.NOTE_07 ) == 1 ) {
			//p.background( _r, _g, _b );
		}
		if ( p.key == 'l' || p.getMidi().midiPadIsOn( MidiWrapper.PAD_08 ) == 1 || p.getMidi().midiPadIsOn( MidiWrapper.NOTE_08 ) == 1 || p.getOsc().oscMsgIsOn( OscWrapper.MSG_LINES ) == 1 ) {
			pickStroke();
		}
