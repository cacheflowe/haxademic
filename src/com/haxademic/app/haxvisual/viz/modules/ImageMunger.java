...
implements IVizModule
{
	...
	public void handleKeyboardInput()
	{
		...
		if ( p.key == 'b' || p.key == 'B' || p.getMidi().midiPadIsOn( MidiWrapper.PAD_08 ) == 1 || p.getMidi().midiPadIsOn( MidiWrapper.NOTE_08 ) == 1 ) {
			newMode();
			randImage();
			newCamera();
		}
		...
		if ( p.key == '1' || p.getMidi().midiPadIsOn( MidiWrapper.PAD_09 ) == 1 || p.getMidi().midiPadIsOn( MidiWrapper.NOTE_09 ) == 1 ) {
			...
			_roundingMode = 2;
		} else if ( p.key == '4' || p.getMidi().midiPadIsOn( MidiWrapper.PAD_12 ) == 1 || p.getMidi().midiPadIsOn( MidiWrapper.NOTE_12 ) == 1 ) {
			_roundingMode = 3;
		}
