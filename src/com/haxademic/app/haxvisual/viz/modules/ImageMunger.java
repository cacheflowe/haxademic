...
implements IVizModule
{
	...
	public void handleKeyboardInput()
	{
		...
		if ( p.key == 'b' || p.key == 'B' || p._midi.midiPadIsOn( MidiWrapper.PAD_08 ) == 1 || p._midi.midiPadIsOn( MidiWrapper.NOTE_08 ) == 1 ) {
			newMode();
			randImage();
			newCamera();
		}
		...
		if ( p.key == '1' || p._midi.midiPadIsOn( MidiWrapper.PAD_09 ) == 1 || p._midi.midiPadIsOn( MidiWrapper.NOTE_09 ) == 1 ) {
			...
			_roundingMode = 2;
		} else if ( p.key == '4' || p._midi.midiPadIsOn( MidiWrapper.PAD_12 ) == 1 || p._midi.midiPadIsOn( MidiWrapper.NOTE_12 ) == 1 ) {
			_roundingMode = 3;
		}
