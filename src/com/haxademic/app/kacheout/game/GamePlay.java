...
//		p.rotateX( p.PI / 16f );
	...
	protected void updateControls() {
		...
		if( p.kinectWrapper != null && p.kinectWrapper.isActive() == true ) {
			...
			float leapFactor = ((float)p.width/(float)KinectSize.WIDTH);
...
//		float xTravel = p.gameWidth() - KinectSize.WIDTH;
		float scale = 1;
		p.translate( -KinectSize.WIDTH/2f, 0, -1000 );
...
//		p.image(p.kinectWrapper.getDepthImage(),0,0);
	...
	protected void drawDebugLines() {
		...
		p.pushMatrix();
		DrawUtil.setCenter( p );
		p.translate( -KinectSize.WIDTH/2, 0, -700 );
