...
//			_cdLogoLoc.setTargetY( 90 );
	...
	protected void drawObjects() {
		...
		drawObjectAtLoc( p.meshPool.getMesh( KacheOut.CREATE_DENVER_LOGO ), _cdLogoLoc.x(), _cdLogoLoc.y(), 0, WHITE.toARGB() );
		drawObjectAtLoc( p.meshPool.getMesh( KacheOut.CREATE_DENVER_TEXT ), _cdTextLoc.x(), _cdTextLoc.y(), 0, WHITE.toARGB() );
		
		// draw kacheout logo
		int kacheOutAnim = p.frameCount % 40;
		if( kacheOutAnim < 20 ) {
			drawObjectAtLoc( p.meshPool.getMesh( KacheOut.KACHEOUT_LOGO ), _kacheOutLoc.x(), _kacheOutLoc.y(), 0, WHITE.toARGB() );
		} else {
			drawObjectAtLoc( p.meshPool.getMesh( KacheOut.KACHEOUT_LOGO_ALT ), _kacheOutLoc.x(), _kacheOutLoc.y(), 0, WHITE.toARGB() );
		}
