...
implements IWebCamCallback, IImageSequenceRecorderDelegate, IScriptCallback {
	...
	protected void drawApp() {
		...
		if (KeyboardState.keyTriggered('r')) {
			recorder.saveToDisk();
			// recorder.saveToDisk(FileUtil.haxademicOutputPath() +
			// "frames-camera-3");
		}
		...
		ImageUtil.cropFillCopyImage(recorder.imageAtFrame(p.frameCount / recordFrameSkip), p.g, p.width / 2, 0, p.width / 2,
				p.height, true);
		recorder.updateSave();
