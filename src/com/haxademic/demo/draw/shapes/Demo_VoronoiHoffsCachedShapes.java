...
//		Config.setProperty( AppSettings.SCREEN_Y, 0 );
	...
	protected void firstFrame() {
		...
		blurMap = PG.newPG(pg.width/4, pg.height/4);
...
//			pg.fill(i % 2 == 0 ? 0 : 255);
			...
			float xIndex = MathUtil.gridColFromIndex(i, gridRes);
			float yIndex = MathUtil.gridRowFromIndex(i, gridRes);
			if(xIndex % 2 == 0) {
				xIndex -= 0.5f;
				yIndex += 0.5f;
			}
			...
			*/
			...
			float rads = p.frameCount * 0.001f + i * segmentRads; 
			float radius = i * 1.5f;
			if(i%2 == 0) {
				cells[i].setPosition(pg.width/2 + P.cos(rads) * radius, pg.height/2 + P.sin(rads) * radius);
			} else {
				cells[i].setPosition(pg.width/2 + P.cos(-rads) * radius/1.f, pg.height/2 + P.sin(-rads) * radius/1.f);
			}
			*/
...
//			float radius = P.floor(i / numVertices) * segmentRads * 200f;
