...
extends PAppletHax {
	...
	protected int positionBufferSize = 1024;
	...
	protected PShaderHotSwap particleVerticesShader;
	int FRAMES = 300;
	...
	protected void firstFrame() {
		...
		// load shader
		particleVerticesShader = new PShaderHotSwap(
	...
	protected void drawApp() {
		...
		// draw shape w/shader
		particleVerticesShader.shader().set("width", (float) positionBufferSize);
		particleVerticesShader.shader().set("height", (float) positionBufferSize);
		particleVerticesShader.shader().set("scale", (p.width / (float) positionBufferSize) * 0.5f * (1f + 0.1f * P.sin(p.frameCount * 0.01f)));
		particleVerticesShader.shader().set("positionMap", positionsBuffer);
		particleVerticesShader.shader().set("pointSize", 6f + 2f * P.sin(p.frameCount * 0.01f));
		particleVerticesShader.update();
		...
		renderedParticles.shader(particleVerticesShader.shader());  	// render particles
		renderedParticles.shape(shape);
