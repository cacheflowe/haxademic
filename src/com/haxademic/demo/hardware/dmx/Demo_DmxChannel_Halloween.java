...
public class Demo_DmxChannel_Halloween
extends PAppletHax {
	...
	protected int lightningFrame = 100;
...
//		}
		...
		switch (testMode) {
			...
			case RGB:
				...
				break;
			case RGB_ALL:
				int lightningWindow = 16;
				float breatheOsc = FrameLoop.osc(0.08f, 0.5f, 1f);
				for (int i = 1; i < 512; i+=3) {
					dmx.setValue(i+0, P.round(valueR * breatheOsc));
					dmx.setValue(i+1, P.round(valueG * breatheOsc));
					dmx.setValue(i+2, P.round(valueB * breatheOsc));
					if(p.frameCount > lightningFrame - lightningWindow) {
						if(MathUtil.randBooleanWeighted(0.3f)) {
							dmx.setValue(i+0, 255);
							dmx.setValue(i+1, 255);
							dmx.setValue(i+2, 255);
						}
					}
				}
				if(p.frameCount >= lightningFrame + lightningWindow) {
					lightningFrame = p.frameCount + (int) p.random(240, 660);
				}
				DebugView.setValue("lightningFrame", lightningFrame);
				debugInfo += "ValueR: " + valueR + "\n\n";
				debugInfo += "ValueG: " + valueG + "\n\n";
				debugInfo += "ValueB: " + valueB + "\n\n";
